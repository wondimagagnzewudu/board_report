...
export default function HOPR_update(props) {
  var data_passed=props.props;
  console.log('props',props);
  console.log('props.hopr_max',props.props.hopr_max[0]);
  ...
  const [winner_balot, setwinner_balot] = useState(props.props.hopr_max[0].candidteid)
  
  const language = [
				
    { values:data_passed.q1,id: 1, value: 'በምርጫ ክልሉ ውስጥ የሚገኙ የምርጫ ጣቢያዎች ብዛት',type:'number' },
    { values:data_passed.q1,id: 2, value: 'በምርጫ ክልሉ ውስጥ የሚገኙ የምርጫ ጣቢያዎች ብዛት',type:'number' },
    { values:0,id: 3, value: 'ውጤት ላይ ያልተካተቱ የምርጫ ጣቢያዎች ቁጥር' ,type:'number'},
    { values:0,id: 4, value: 'ምክንያት' ,type:'text'},
    { values:0,id: 5, value: 'በምርጫ ክልሉ የተመዘገቡ አጠቃላይ የመራጮች ቁጥር',type:'number'},
  ...
  const languageEnglish = [
    ...
    { values:data_passed.q1,id: 1, value: 'Number of polling stations in the constituency',type:'number'  },
    { values:data_passed.q1,id: 2, value: 'Number of polling stations in the constituency' ,type:'number' },
    { values:0,id: 3, value: 'Number of polling stations exclused from the results',type:'number'  },
    { values:0,id: 4, value: 'Reason',type:'text'  },
    { values:0,id: 5, value: 'Total number of registered voters in the constituency',type:'number'  },
...
candidate_data.map((id, ITEM) => (
        ...
        >
          ...
          {candidate_data.map((id, ITEM) => (
              <Option  size={'large'} key={ITEM} value={id.candidatename}>{id.candidatename},{'/'},{ id.partyname}</Option>
          ))}
          </Select>
        </Form.Item>
